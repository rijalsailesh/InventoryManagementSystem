...
namespace Digitalkirana.DataAccessLayer
{
    public class CategoryDAL
    {
        ...
        public bool UpdateCategory(CategoryBLL category)
        {
            try
            {
                string query = $"UPDATE category_tbl SET CategoryName = '{category.CategoryName}', Description = '{category.Description}', AddedDate = '{category.AddedDate}' WHERE Id = {category.Id}";
                MySqlCommand cmd = new MySqlCommand(query, con);
                con.Open();
                int result = cmd.ExecuteNonQuery();
                if (result == 1)
